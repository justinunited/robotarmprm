...
def sendPreset(index, ser):
    arr = [[0, 0, 1.0471975511965976, -1.0471975511965976, 0], [0.39758303053672206, -0.06465660677361162, 0.7778999842793566, -0.713243377505745, 0.003740887808553861], [-1.613508521610387, -0.43845774116827085, 1.5193817720427678, -1.080924030874497, 0.1101420978636034], [-2.1998448397653356, -0.2940124548248866, 0.6072300149307314, -0.3132175601058448, 0.7842813459080811], [-2.1848763158741162, -0.9036277714903422, 0.2813298175220469, 0.6222979539682953, 0.6140799890792197]]
    arr = (np.array(arr)*180/pi).tolist()
...
def pathTraversal(initial, final, type, mode, ser):
    ...
    if initial == 'home':
        q1_i, q2_i, q3_i, q4_i, q5_i = 0, 0, pi/3, -pi/3, 0
    else:
        task_init = setpointDict[str(initial)]
        q1_i, q2_i, q3_i, q4_i, q5_i = inverseKinematics(task_init[0], task_init[1], task_init[2], task_init[3])
    if final == 'home':
        q1_f, q2_f, q3_f, q4_f, q5_f = 0, 0, pi/3, -fpi/3, 0
    else:
